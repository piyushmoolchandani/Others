...
public class NewHttpServer {
    ...
    private static final Tracer tracer =
            openTelemetry.getTracer("example-server","1.0.0");
    ...
    private NewHttpServer() throws IOException {
        this(port);
    }

    private NewHttpServer(int port) throws IOException {
    ...
    private static OpenTelemetry initOpenTelemetry() {
        ...
        JaegerGrpcSpanExporter jaegerExporter =
                JaegerGrpcSpanExporter.builder()
                        .setServiceName("ServerClientServerExample")
    ...
    private static class HelloHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            Span span =
                    tracer.spanBuilder("childSpanServerSide").setParent(context).setSpanKind(Span.Kind.SERVER).startSpan();

            try (Scope scope = span.makeCurrent()) {
                span.setAttribute("component", "http");
                span.setAttribute("http.method", "GET");
                span.setAttribute("http.scheme", "http");
                span.setAttribute("http.host", "localhost:" + NewHttpServer.port);
    ...
    public static void main(String[] args) throws Exception {
        final NewHttpServer s = new NewHttpServer();
